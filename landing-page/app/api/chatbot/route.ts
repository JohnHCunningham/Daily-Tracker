...
export async function POST(request: Request) {
  try {
    ...
    // Generate embedding for the user's question using OpenAI (graceful fallback)
    let contextFromKB = ''
    try {
      ...
      // OpenAI or Supabase unavailable — continue without RAG context
      console.warn('RAG search skipped (OpenAI/Supabase unavailable):', embeddingError)
    }
    ...
    if (leadCaptureMatch) {
      ...
      // Send email notification via API call to main app
      if (leadData.email) {
        try {
          await fetch('https://app.oneclickcoaching.com/api/lead-notification', {
            method: 'POST',
